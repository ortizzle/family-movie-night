...
function parseClaudeJson(text){
  ...
  if (a < 0 || b < a) throw new Error('BAD_SHAPE');
  try { return JSON.parse(t.slice(a, b + 1)); }
  catch (err){ throw new Error('BAD_SHAPE'); }
}
/* one sentence a parent can act on, for whatever came back */
function claudeTrouble(e){
  var m = (e && e.message) || '';
...
function generatePack(night){
  ...
  return askClaude(prompt, { system: system, maxTokens: 2200 }).then(function(text){
    var obj = parseClaudeJson(text);
