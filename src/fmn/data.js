...
var Sync = {
  ...
  save: function(localData){
    ...
    return self.pullRemote()
      ...
      .then(function(res){
        ...
        /* Fold in anything written while the request was open. A round trip
           takes a second or two, and a reaction typed in that window used to
           be wiped by this very line: the merge was computed before the save
           happened, so writing it back deleted it locally — while the upload
           already on its way carried it to everyone else's phone. Merging
           again against what's on disk *now* is safe, because the merge is
           newest-wins per record and anything newer is exactly what we want
           to keep. */
        mergedHolder = self.merge(Store.get('data', { records:{} }), mergedHolder);
  ...
  load: function(){
    var self = this;
    /* read local *after* the fetch, never before — see the note in save() */
    return self.pullRemote().then(function(remote){
      var merged = self.merge(Store.get('data', { records:{} }), remote);
...
var syncTimer = null;
var syncBusy = false;
/* Something was saved while a sync was already running. The scheduled retry
   would have hit the busy guard and given up, leaving that write sitting on
   this phone until somebody happened to save again. */
var syncAgain = false;
...
function runSync(){
  ...
  Sync.save(data).then(function(merged){
    syncBusy = false;
    adoptMerged(merged);
    // a write landed mid-flight; it isn't on the gist yet
    if (syncAgain) scheduleSync();
  }).catch(function(){
    syncBusy = false;
...
function nextOpenFriday(){
  ...
  /* Tonight's Friday is spent once its movie has played. Without this the
     lineup keeps offering today's date, and anything genuinely sooner — a
     Sunday bonus, say — looks further out than the slot that already went.
     A bonus doesn't spend the Friday; it never took the turn. */
  if (nf === AZ.today() && turnNights(happenedNights()).some(function(n){ return n.date === nf; })){
    nf = AZ.addDays(nf, 7);
  }
